...
if __name__ == '__main__':
    ...
    parser.add_argument("--train-dataset-file-name", type=str,
                        default="dataset/2021_1_data/total_features_to_train.csv")
    parser.add_argument("--train-features-file-name", type=str, default="dataset/2021_1_data/train_features.csv")
    parser.add_argument("--train-labels-file-name", type=str, default="dataset/2021_1_data/train_labels.csv")
    args = parser.parse_args()

    raw_data_file_name = args.train_dataset_file_name
    assert os.path.exists(raw_data_file_name), "[ERROR] must provide an existed feature and label dataset"
    feature_file_name = args.train_features_file_name
    label_file_name = args.train_labels_file_name
    feature_titles, label_titles = generate_titles()
    with open(raw_data_file_name, "r") as fp, open(feature_file_name, "w") as gp1, open(label_file_name, "w") as gp2:
        gp1.write("|".join(feature_titles) + "\n")
        gp2.write("|".join(label_titles) + "\n")
